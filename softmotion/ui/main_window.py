...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.import_button.setToolTip("导入 PNG / JPG / WebP · Ctrl+O")
        self.import_button.clicked.connect(self.choose_image)
        header.addWidget(self.import_button)
        self.export_button = QPushButton("导出素材")
        ...
        for label, panel in [("动态效果", self.motion_panel), ("背景与裁剪", self.canvas_panel)]:
            scroll = QScrollArea()
        ...
        self.reset_button.setToolTip("重置动态参数并暂停归位，保留背景与裁剪")
        ...
        self.statusBar().showMessage("就绪 · 支持透明图片")
        status_note = QLabel("本地处理  ·  PNG / JPG / WebP")
    ...
    def refresh_canvas_label(self):
        ...
        background = {"transparent": "透明", "solid": "纯色", "image": "图片背景"}[scene.background_mode]
        self.canvas_label.setText(f"{scene.width} × {scene.height}  ·  {background}")

    def choose_image(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "导入静态图片", "", "图片 (*.png *.jpg *.jpeg *.webp)")
        if path:
    ...
    def update_playback(self, playing):
        ...
        self.statusBar().showMessage("播放中 · 实时预览" if playing else "已暂停" if loaded else "就绪")
    ...
    def closeEvent(self, event):
        if self.export_worker is not None and self.export_worker.isRunning():
            self.statusBar().showMessage("请等待导出完成，或先取消导出再关闭窗口。")
            event.ignore()
            return
        self.preview.pause()
    ...
    def choose_export(self):
        ...
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        self.export_button.setEnabled(False)
        self.export_progress = QProgressDialog("正在生成动画帧…", "取消", 0, 100, self)
        self.export_progress.setWindowTitle("导出")
    ...
    def cancel_export(self):
        if self.export_worker is not None:
            ...
            self.statusBar().showMessage("正在取消导出…")

    def export_succeeded(self, path):
        self.export_progress.close()
        QMessageBox.information(self, "导出完成", f"已保存至：\n{path}")
    ...
    def export_cancelled(self):
        self.export_progress.close()
        self.statusBar().showMessage("已取消导出，临时文件已清理。")
