...
def render_frame(source, viewport, size, phase, settings, transparent, scene=None, mp4_background=None,
                 character=None, character_renderer=None):
    canvas = QImage(*size, QImage.Format.Format_RGBA8888)
    if canvas.isNull():
        raise MemoryError("无法分配导出画布，请降低尺寸。 Cannot allocate the export canvas. Reduce its dimensions.")
    ...
    try:
        if scene is not None:
            ...
            draw_scene(painter, source, scene, phase, settings, mp4_background, character, character_renderer)
        else:
            scale = min(size[0] / viewport[0], size[1] / viewport[1])
            painter.translate((size[0] - viewport[0] * scale) / 2,
                              (size[1] - viewport[1] * scale) / 2)
            painter.scale(scale, scale)
            draw_artwork(painter, source, viewport, phase, settings, character=character,
...
def export_animation(source, viewport, settings: MotionSettings, options: ExportOptions,
                     ...
                     scene=None, mp4_background=None, character=None):
    ...
    if options.format not in {"gif", "mp4", "png"}:
        raise ValueError("不支持的导出格式。 Unsupported export format.")
    if (source.isNull() or min(viewport) < 64 or not 64 <= options.long_edge <= 1920
        ...
        raise ValueError("无效的图片或导出参数。 Invalid image or export settings.")
    if options.format == "png" and destination.exists():
        raise ValueError("PNG 序列需要一个新的文件夹，请更换名称。 PNG sequences require a new folder. Choose another name.")
    size = options.size(viewport)
    if scene is not None:
        if not (64 <= scene.width <= 1920 and 64 <= scene.height <= 1920):
            raise ValueError("画布宽高需要在 64–1920 像素之间。 Canvas dimensions must be between 64 and 1920 pixels.")
        if scene.background_mode not in {"transparent", "solid", "image"}:
            raise ValueError("无效的背景类型。 Invalid background type.")
        if scene.background_mode == "image" and scene.background_image.isNull():
            raise ValueError("请先选择背景图片，或切换到透明 / 纯色背景。 Select a background image, or switch to a transparent or solid background.")
        if scene.crop is not None:
            x, y, w, h = scene.crop
            if x < 0 or y < 0 or w < 1 or h < 1 or x + w > source.width() or y + h > source.height():
                raise ValueError("裁剪区域超出图片范围。 The crop area exceeds the image bounds.")
        ...
        if options.format == "mp4" and scene.background_mode == "transparent" and mp4_background is None:
            raise ValueError("MP4 不支持透明背景，请选择 MP4 填充颜色。 MP4 does not support transparency. Select an MP4 fill color.")
    duration, count, actual_fps = options.timing(settings)
    character_renderer = CharacterRenderer(source, character) if character is not None else None
    if options.format == "gif" and size[0] * size[1] * count > 150_000_000:
        raise ValueError("GIF 帧数据较大，请降低尺寸、帧率或循环次数（建议 512 px / 15 fps）。 GIF frame data is too large. Reduce dimensions, frame rate, or loops (suggested: 512 px / 15 fps).")
    ...
    with tempfile.TemporaryDirectory(prefix=".softmotion-", dir=destination.parent) as staging:
        ...
        try:
            ...
            if options.format == "gif":
                ...
                if code != 0:
                    errors.seek(0)
                    raise RuntimeError("MP4 编码失败 MP4 encoding failed: " + errors.read().decode("utf-8", errors="replace")[-2000:])
                if not output.exists() or output.stat().st_size == 0:
                    raise RuntimeError("MP4 编码器未生成有效文件。 The MP4 encoder did not produce a valid file.")
            else:
